...
template <usize N, typename _CharT = char, typename Traits = std::char_traits<_CharT>>
class fs_string
{
public:
//typedefs
...
public:
//index operator
    inline constexpr _CharT& operator[](const size_type& idx)
    {
        if(idx < len)
            return m_data[idx];
        else
    ...
    inline constexpr const _CharT& operator[](const size_type& idx) const
    {
        if(idx < len)
            return m_data[idx];
        else
...
//equals operator
    ...
    inline friend constexpr bool operator==(const fs_string<N, _CharT>& lhs, const fs_string<M, _CharT>& rhs)
    {
        return N == M && !traits::compare(lhs.data(), rhs.data(), len);
    }
    template <size_type M>
    inline friend constexpr bool operator==(const fs_string<N, _CharT>& lhs, const _CharT(&rhs)[M])
    {
        return N == M && !traits::compare(lhs.data(), &rhs[0], len);
    }
    template <size_type M>
    inline friend constexpr bool operator==(const _CharT(&lhs)[N], const fs_string<M, _CharT>& rhs)
    {
...
//find
    ...
    template <size_type M>
    inline constexpr size_type find(const fs_string<M, _CharT>& s, const size_type& pos = 0) const
    {
    ...
    template <size_type M>
    inline constexpr size_type find(const _CharT(&str)[M], const size_type& pos = 0) const
    {
        return this->find(fs_string<M, _CharT>(str), pos);
    }
public:
//find_first_of
    ...
    template <size_type M>
    inline constexpr size_type find_first_of(const fs_string<M, _CharT>& s, const size_type& pos = 0) const
    {
        if(!check_len(pos, 0, len) || !check_len(pos + s.length() - 1, 0, len))
            return npos;
        for(size_type i = pos; i < len; i++)
    ...
    template <size_type M>
    inline constexpr size_type find_first_of(const _CharT(&str)[M], const size_type& pos = 0) const
    {
        return this->find_first_of(fs_string<M, _CharT>(str), pos);
    }
public:
//find_last_of
    ...
    template <size_type M>
    inline constexpr size_type find_last_of(const fs_string<M, _CharT>& s, const size_type& pos = npos) const
    {
        size_type i = std::min(pos, len - 1);
        for(; i >= 0; i--)
            if(s.find((*this)[i]) != npos)
    ...
    template <size_type M>
    inline constexpr size_type find_last_of(const _CharT(&str)[M], const size_type& pos = npos) const
    {
        return this->find_last_of(fs_string<M, _CharT>(str), pos);
    }
public:
//find_first_not_of
    ...
    template <size_type M>
    inline constexpr size_type find_first_not_of(const fs_string<M, _CharT>& s, const size_type& pos = 0) const
    {
        if(!check_len(pos, 0, len) || !check_len(pos + s.length() - 1, 0, len))
            return npos;
        for(size_type i = pos; i < len; i++)
    ...
    template <size_type M>
    inline constexpr size_type find_first_not_of(const _CharT(&str)[M], const size_type& pos = 0) const
    {
        return this->find_first_not_of(fs_string<M, _CharT>(str), pos);
    }
public:
//find_last_not_of
    ...
    template <size_type M>
    inline constexpr size_type find_last_not_of(const fs_string<M, _CharT>& s, const size_type& pos = npos) const
    {
        size_type i = std::min(pos, len - 1);
        for(; i >= 0; i--)
            if(s.find((*this)[i]) == npos)
    ...
    template <size_type M>
    inline constexpr size_type find_last_not_of(const _CharT(&str)[M], const size_type& pos = npos) const
    {
        return this->find_last_not_of(fs_string<M, _CharT>(str), pos);
    }
public:
//substr
    template <size_type M>
    inline constexpr fs_string<M, _CharT> substr(const size_type& pos = 0) const
    {
        assert(M > N || !check_len(pos, 0, len));
        fs_string<M, _CharT> res;
...
public:
//ostream & istream
    inline friend std::basic_ostream<_CharT>&
    operator<<(std::basic_ostream<_CharT>& os, const fs_string<N, _CharT>& s)
    {
        os.write(s.data(), s.length());
        return os;
    }
    inline friend std::basic_istream<_CharT>&
    operator>>(std::basic_istream<_CharT>& is, fs_string<N, _CharT>& s)
    {
